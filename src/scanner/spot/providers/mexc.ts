...
export class MEXCSpotProvider {
  ...
  async fetchPrices(): Promise<MarketPrice[]> {
    try {
      ...
      console.log(`✅ [MEXC] Fetched ${prices.length}/${SPOT_SYMBOLS.length} symbols.`);
      return prices;
    } catch (err) {
      console.error('❌ [MEXC] Provider failed:', err instanceof Error ? err.message : err);
      return [];
    }
  }
  ...
  private async fetchSymbol(symbol: SpotSymbol): Promise<MarketPrice | null> {
    try {
      ...
      if (!res.ok) {
        console.warn(`⚠️ [MEXC] HTTP ${res.status} for ${symbol}`);
        return null;
      }
      ...
      if (!bidPrice || !askPrice || isNaN(bidPrice) || isNaN(askPrice)) {
        console.warn(`⚠️ [MEXC] Invalid price data for ${symbol}`);
        return null;
      }
      ...
      console.error(`❌ [MEXC] Failed to fetch ${symbol}:`, err instanceof Error ? err.message : err);
      return null;
    }
  }
}
